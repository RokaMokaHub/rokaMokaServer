...
public abstract class ApiResponseWrapper {
    ...
    protected ResponseEntity<ApiResponse<Void>> success() {
        ...
        ApiResponse<Void> body = new ApiResponse<>(ctx);

        ServiceContext.clearContext();
        return ResponseEntity.ok().headers(headers).body(body);
    }

    protected <T> ResponseEntity<ApiResponse<T>> success(T response) {
        ...
        HttpHeaders headers = buildResponseHeaders(ctx);
        ApiResponse<T> body = new ApiResponse<>(ctx, response);

        ServiceContext.clearContext();
        return ResponseEntity.ok().headers(headers).body(body);
    }

    protected ResponseEntity<ApiResponse<Void>> error(Exception e, HttpStatus status) {
        log.error("", e);
