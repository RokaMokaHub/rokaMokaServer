...
public class UserRestController extends ApiResponseWrapper {

    private final UserService userService;
    private final AuthenticationService authenticationService;

    @PostMapping("/register")
    public ResponseEntity<ApiResponse<UserResponseDTO>> register(@RequestBody UserBasicDTO userDTO)
            throws RokaMokaContentDuplicated {
        return success(userService.createNormalUser(userDTO));
    }

    @PostMapping("/login")
    public ResponseEntity<ApiResponse<UserResponseDTO>> login(Authentication authentication) {
