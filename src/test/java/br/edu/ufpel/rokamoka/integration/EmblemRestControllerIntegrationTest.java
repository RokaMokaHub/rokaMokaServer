...
public class EmblemRestControllerIntegrationTest extends BaseIT {
        ...
        public void consultaDeEmblemaPorId_deveRetornarTodasAsObras_seUsuarioTiverEmblema() throws Exception {
                ...
                this.webTestClient.perform(
                                get("/emblems/{id}", setup.idEmblema())
                ...
                for (int index = 0; index < artworks.size(); index++) {
                        ...
                        this.webTestClient.perform(
                                        ...
                                        .andExpect(jsonPath("$.body.artworks[" + index + "].nome")
                                                        .value(artwork.getNome()))
                                        .andExpect(jsonPath("$.body.artworks[" + index + "].nomeArtista")
                                                        .value(artwork.getNomeArtista()))
                                        .andExpect(jsonPath("$.body.artworks[" + index + "].descricao")
                                                        .value(artwork.getDescricao()))
                                        .andExpect(jsonPath("$.body.artworks[" + index + "].link")
                                                        .value(artwork.getLink()))
                                        .andExpect(jsonPath("$.body.artworks[" + index + "].qrCode")
                                                        .value(artwork.getQrCode()));
                }
        }
        ...
        @Test
        public void consultaDeEmblemPorExibicaoId_DeveRetornarOK_QuandoEmEmblemaExisir() throws Exception {
                EmblemSetupData setup = this.setupUsuarioComEmblemaDaExposicao();

                this.webTestClient.perform(
                                get("/emblems")
                                                .param("exhibitionId", String.valueOf(setup.idExposicao))
                                                .with(jwt().jwt(jwt -> jwt.subject(setup.nomeUsuario()))))
                                .andExpect(status().isOk());
        ...
        @Test
        public void consultaDeEmblemPorExibicaoId_DeveRetornar4xx_QuandoNaoFoiEnviadoExibicaoId() throws Exception {
                EmblemSetupData setup = this.setupUsuarioComEmblemaDaExposicao();
